...
const RouteData = ({ routes, mode, updateRoute }) => {
  ...
  return (
    <div className="w-[22rem] px-2 h-full overflow-hidden">
      <div className="mb-2 bg-bg rounded-lg p-4 w-full">
        <div className="flex">
          <div
            className=" text-darkSlate mr-6 h-10 w-10 bg-white rounded-full 
          ...
          <div className=" w-40 p-1 rounded-3xl border-[1.5px] bg-darkbg border-lightSlate flex items-center">
            {routes.map((route, index) => {
              return (
                <div
                  key={index}
                  className={`rounded-3xl flex items-center justify-center p-[2px] cursor-pointer w-full
                   text-lightSlate text-sm 
