...
const Directions = ({ show, setShow, map }) => {
  ...
  const handleClick = (suggestion) => {
    ...
    new mapboxgl.Marker(el).setLngLat(suggestion.geometry.coordinates).addTo(map);
  };
      ...
      async function fetchRoute() {
        ...
        try {
          const res = await axios.get(
            ...
            // profile: Possible values are mapbox/driving-traffic, mapbox/driving, mapbox/walking, or mapbox/cycling
  ...
  return (
    <div
      className={`w-[22rem] max-h-screen bg-darkbg bg-opacity-90 flex flex-col transition-all absolute top-0
       rounded-md m-1 
       ${show ? "left-0" : "left-[-25rem]"}`}
    >
      <div ref={heightRef}>
        <div className="flex items-center justify-between px-4 text-lightSlate mt-2">
          <MdClose size={25} className=" cursor-pointer" onClick={() => setShow(false)} />
          <BiRefresh size={25} className=" cursor-pointer" onClick={() => reset()} />
        </div>
      ...
      <div
        className="w-[17rem] ml-10 mx-4 mb-4 my-2 p-1 rounded-3xl border-[1.5px] bg-darkbg
      border-lightSlate flex items-center"
      >
        {modes.map((item, index) => (
          <div
            key={index}
            className={`rounded-3xl flex items-center justify-center whitespace-nowrap p-[2px] 
            cursor-pointer w-full text-lightSlate text-sm 
      ...
      {routes?.length > 0 && <RouteData routes={routes} mode={mode} updateRoute={updateRoute} />}
      {suggestions?.length > 0 && (
        <Suggestions map={map} suggestions={suggestions} handleClick={handleClick} dir={true} />
      )}
    </div>
  );
};
